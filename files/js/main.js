...
const sendForm = document.querySelector('#everad_form')
$("#everad_form").on("submit", function (event) {
    event.preventDefault();

    const flow_hash = 'UvYk';
    const country = 'ro';
    const landing = 'http://ro.urotrinev.com/'
    const referrer = 'null';
    ...
    const nameAndPhone = $(this).serialize();
    const serialized = nameAndPhone + '&' + $.param(obj)

    // $.post('https://wapi.leadbit.com/api/new-order/_5f882f3199bec124269446', serialized)
    $.ajax({
        crossDomain: true,
        type: 'POST',
        url: 'https://wapi.leadbit.com/api/new-order/_5f882f3199bec124269446',
        data: serialized,
        headers: {
...
function Datee() {
    var msInDay = 86400000,
        counterLength = 33,
        months, countryName = 'ro', // Встановлюємо мову для місяців.
    ...
    for (var counter = 0; counter < counterLength; counter++) {
        ...
        for (var i = 0; i < nodeList.length; i++) {
            ...
            if (nodeList[i].className.match(/\btime\b/)) {
                nodeList[i].innerHTML += " în " + timeArray[timeCounter];
                timeCounter++;
            }
        }
    }
